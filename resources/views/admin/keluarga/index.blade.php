...
<head>
    ...
    <title>Data Keluarga - Admin</title>
    <style>
...
<body>
    <div class="wrap">
        <div class="row">
            <h2 style="margin:0;">Data Keluarga Lansia</h2>
            <div>
                <a class="btn" href="{{ route('admin.keluarga.create') }}">Tambah</a>
            ...
            <tbody>
                @forelse ($keluarga as $k)
                    <tr>
                        ...
                        <td>{{ $k->alamat }}</td>
                        <td>{{ $k->hubungan }}</td>
                        <td>{{ $k->lansia->nama_lansia ?? '-' }}</td>
                        <td>
                            <a class="btn" href="{{ route('admin.keluarga.edit', $k) }}">Edit</a>
                            <form method="POST" action="{{ route('admin.keluarga.destroy', $k) }}" onsubmit="return confirm('Hapus data keluarga ini?')">
                                @csrf
                                @method('DELETE')
